...
<form class="form" action="index.php?view=meta-gked&id=<?php echo $data['IDActivity']; ?>&action=edit" method="POST">

    <div class="caption">Редактирование ГКЭД</div>

    <input type="hidden" name="id" value="<?php echo $data['IDActivity'] ?>" />

    <div class="field">
        <label class="fixed">ГКЭД <span class="required">*</span>:</label>
        <input type="text" name="gked" value="<?php echo htmlspecialchars($_POST['gked'] ?? $data['Gked']); ?>" placeholder="Укажите ГКЭД" required>
    ...
    <div class="field">
        ...
        <input type="text" name="parent" value="<?php echo htmlspecialchars($_POST['parent'] ?? $data['ParentGked']); ?>" placeholder="Укажите родительский ГКЭД" required>
